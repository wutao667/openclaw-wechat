...
class UserNameCache {
    nameByOpenId = new Map();
    openIdsByName = new Map();
    maxSize;
    ttlMs;
    chatBots = new Map();
    chatMembers = new Map();
    ...
    recordChatBots(chatId, members) {
        const enriched = members.map((m) => ({ ...m, kind: 'bot' }));
        ...
        for (const m of enriched) {
            if (m.openId)
                this.writeEntryNoEvict(m.openId, m.name, 'bot');
        }
        this.evict();
    }
    recordChatMembers(chatId, members) {
        ...
        for (const m of enriched) {
            if (m.openId)
                this.writeEntryNoEvict(m.openId, m.name, m.kind);
        }
        this.evict();
    }
    getChatBots(chatId) {
    ...
    // ----- private helpers -----
    writeEntry(openId, name, kind) {
        this.writeEntryNoEvict(openId, name, kind);
        this.evict();
    }
    writeEntryNoEvict(openId, name, kind) {
